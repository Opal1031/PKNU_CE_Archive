...
void Song::register_playlist(PlayList* ptr_plist) {
    ...
    cout << title << " by " << artist << " added to playlist " << ptr_plist->get_name() << endl;
}

void Song::print(ostream& out) {
    out << "  " << id << ":" << title << ":" << artist << ":" << album << ":" << mv_url << ":";
...
void Song::unregister_playlist(string plist_name) {
    for (auto it = playlists.begin(); it != playlists.end(); it++) {
        if ((*it) -> get_name() == plist_name) {
            ...
            cout << title << " by" << artist << " deleted from playlist " << plist_name << endl;
            return;
        }
    }
